...
class PooledErrorCompute(object):
    ...
    def evaluate_genomes(self, genomes, config):
        ...
        for score, episode in episodes:
            ...
            N = len(episode)
            D = np.sum((np.eye(N, k=i) * gamma ** i for i in range(N)))
    ...
    while 1:
        try:
            ...
            if score_range:
                ...
                plt.plot(S[2], 'g-')
                plt.grid()
                plt.savefig(config_file + "score-ranges.svg")
                plt.close()
            ...
            if solved:
                ...
                for n, g in enumerate(best_genomes):
                    name = config_file + 'll-winner-{0}'.format(n)
                    with open(name+'.pickle', 'wb') as f:
                        pickle.dump(g, f)

                    visualize.draw_net(config, g, view=False, filename=name + "_" + config_file + "-net.gv")
    ...
    for pop in pop_sizes:
      for res in response_mutate_rate:
        for bias in bias_mutate_rate:
          for i in range(5):
            start = time.time()
            run(config_file = 'config_files/config-ll-'+str(pop)+'-'+str(res*100)+'-'+str(bias*100)+'.txt')
            end = time.time()
            with open('time-ll-.txt', 'a') as f:
