...
def run(config_file = '', limit=1000):
    ...
    pop.add_reporter(neat.Checkpointer(1000, 90000))
    # return
...
if __name__ == '__main__':
    ...
    for pop_size in pop_sizes:
        for mutate_rate in mutate_rates:
            for i in range(2):
                start = time.time()
                count, score = run(config_file = 'config_files/config-ll-'+str(pop_size)+'-'+str(mutate_rate*100)+'.txt')
                end = time.time()
                with open('time-ll-neat.txt', 'a') as f:
